...
- **Caveat on this source**: the two supplied PDF parts have a genuine gap (thesis pages ~23-50 were not included) and both stop at page 74 — before Chapter 4's likely numeric "Simulation Parameters" table and before the thesis's own referenced Appendix A ("Table A.1: Maximum Supported APs per DU under 20 Gbps Fronthaul Budget for Split 7.2 and Split 8," page 115), which would very plausibly have given exactly the kind of concrete, split-dependent numeric figures still needed to close the RU/fronthaul flag. Nothing was inferred or guessed to fill this gap; if those pages become available, they are the most promising remaining lead for this flag.
