...
### 6.3 Limitations of the Power, Traffic, and Scenario-Scale Models (thesis-chapter draft)

*The paragraphs below are drafted for direct use in the thesis (Chapter 4, Simulation Results, or Chapter 5, Conclusion and Future Work — see `docs/oran_thesis_guide.md`'s chapter mapping) and summarize the outcome of an extensive, multi-source literature-verification effort undertaken specifically to characterize — rather than silently accept or overclaim — the numeric placeholders in `oran_env/`. Inline citations follow this section's descriptive style; before submission these should be converted to numbered entries in Section 9's reference list.*

The environment's power, traffic, and scenario-scale parameters (`oran_env/power_model.py`, `oran_env/traffic_model.py`, and the default `n_ru`/`n_ue` scenario size) were initially chosen as literature-style placeholders selected only to preserve internally consistent, monotonic trade-offs — for instance, that a more centralized functional split shifts power from the radio unit toward the fronthaul and distributed unit — rather than as physically measured constants. To characterize how well these placeholders reflect real O-RAN systems, we conducted a systematic, multi-round literature search across nine independent verification passes, examining primary standards documents (3GPP TR 38.801, TR 38.864), vendor datasheets (Benetel RAN550), peer-reviewed surveys and measurement studies (e.g., Abubakar et al., Al-Tahmeesschi et al.), master's theses (a CF-mMIMO/O-RAN thesis; a University of Bologna O-RAN CU-scaling thesis), and industry white papers and analytical modeling papers (a Rutgers WINLAB/Open Networking Foundation/ORCID Lab commercial-testbed white paper; Kuaban et al.'s O-DU/O-CU analytical models). This search was exhaustive but not indefinite: it continued until no further independently-supplied source added new, verifiable information, and its findings — never used to silently rescale a constant without a stated, same-units justification — are summarized here rather than presented as settled fact.

**RU/DU/CU/fronthaul power constants.** This remains the least-resolved of the model's placeholder groups: no source examined varies 3GPP functional split option while simultaneously reporting real, component-decomposed (RU vs. DU vs. CU vs. fronthaul) power in Watts at a scale comparable to this model's small-cell scenario — the combination that would be needed to validate the model's per-split arrays directly. That said, several individual constants and structural choices did receive genuine, if partial, support. The power amplifier drain efficiency (`pa_efficiency=0.25`) is corroborated by three independent, real, same-quantity measurements (29-39% and 14-32% for two bands in the Rutgers/ONF/ORCID white paper; 35% in Kuaban et al.'s numerical example), all clustering around this model's value without the scale-mismatch caveat that affects every absolute-Watt constant. The distributed-unit static power (`p_du_static_w=50.0`) numerically matches Kuaban et al.'s own fitted platform power (50 W), though their fuller idle-power floor — including accelerator and minimum-active-core terms — is higher, so this is reported as a partial rather than complete match. The model's implied share of total power attributable to the radio unit (16-36% at the model's default scale) is bracketed, not contradicted, by two independent real measurements that themselves span a wide range with system scale: 29-81% across configurations in the Rutgers/ONF/ORCID white paper, and 66-82% in real macro-cell/massive-MIMO deployments cited by the Bologna thesis — the smaller-scale end of these real ranges sits close to this model's own figure, while the large-scale end does not, suggesting radio-unit power share itself grows with deployment scale rather than being a fixed ratio this model could adopt directly. The qualitative direction of the split-centralization trade-off (more centralized processing implies more fronthaul burden and less radio-unit-side processing) is independently corroborated by multiple sources (Rony et al.; a Trinity College Dublin energy-latency optimization study), and the underlying static-plus-load-dependent functional form is structurally validated against 3GPP TR 38.864's own network-energy-savings model and a real commercial multi-band power-model formula in the Rutgers/ONF/ORCID white paper. What is not validated is the specific numeric ratio this model uses across its three split levels, nor its fronthaul power constants specifically, nor — a structural gap disclosed rather than corrected — the assumption that distributed- and centralized-unit power scales linearly with the number of active radio units; multiple sources instead model sub-linear scaling from statistical multiplexing and control-plane pooling, which this model does not currently capture.

**Traffic model.** The peak Poisson arrival rate and packet size (`lambda_peak`, `packet_size_bits`) are derived directly from 3GPP TR 38.864's own FTP Model 3 traffic definition, a real standardized 3GPP traffic model, and are treated as resolved rather than placeholder values. The model's implied floor-versus-active daily duration split (an 8-hour floor, 16-hour active period) exactly matches the three-period load-weighting scheme in ETSI TR 103 737, cited via a CF-mMIMO/O-RAN thesis, though this confirms only the aggregate day-fraction split and not the four individual trapezoid breakpoints (`t1`-`t4`) or the floor-to-peak ratio (`floor_ratio`) independently, since a three-bucket standard cannot uniquely determine four free parameters.

**Split-to-centralization mapping and default scenario scale.** The three-level split abstraction (`c ∈ {0, 1, 2}`, mapped loosely onto 3GPP Options 2, 6, and 8) is a deliberate tractability simplification rather than a literature-derived mapping; its qualitative monotonic direction is corroborated by 3GPP TR 38.801's own bandwidth and latency tables and by two independent studies of the centralization trade-off, but the model's own power-ratio-versus-bandwidth-ratio relationship remains unreconciled with real fronthaul bandwidth requirements, which scale far more steeply across split options than this model's power constants do. The default scenario scale (`n_ru=4`, `n_ue=8`) sits within the range of comparable published RAN-DRL scenario scales — one thesis's own experimental testbed uses an identical radio-unit count — but remains a tractability-driven choice rather than an independently validated "correct" scale for this problem.

**Mitigation.** Because an ideal source — one that both varies functional split option and reports matching, component-decomposed absolute power — was not found despite this extensive search, the placeholders above are retained with their limitations explicitly disclosed rather than presented as measured facts. [If conducted: a] sensitivity analysis perturbing the power-model constants within the ranges bracketed by the literature above (e.g., an order of magnitude around each RU/DU/CU/fronthaul term) shows that the qualitative comparison between BMPP-DQN and the baseline methods is not an artifact of any single unvalidated constant. [This sentence should be finalized once/if that sensitivity analysis is actually run — see Section 6.1's computational-analysis scope and the "Tomorrow's Plan" note in `docs/daily_log.md`'s most recent O-RAN entry.]
...
**2026-08-30 literature check, part 4**: Tariq, Raj, Pandey, Dzeafaragic, Popovski, and Kilper (Trinity College Dublin / Aalborg University), "Energy–Latency Trade-offs in O-RAN with Distributed Baseband Processing and AI Inference" (arXiv:2608.02082), independently confirms the qualitative trade-off direction this mapping assumes, from a different angle: its own four candidate configurations (G/F/M/B, placing baseband + AI-inference processing progressively deeper from O-RU to a data center) show energy *decreasing* and latency *increasing* as processing centralizes (Fig. 3: ~13.3 mJ/bit / ~0.3 ms at O-RU-only "Conf. G" vs. ~9.9 mJ/bit / ~1.3 ms at data-center "Conf. B", at zero load) — the same "more centralized = lower processing energy, higher transport latency" trade-off this document's own `c` parameter encodes, from an entirely independent throughput-based energy-latency model (not fed by any of the sources already cited above). This paper models baseband-processing/AI-inference *compute* energy in mJ/bit via an abstracted parameterization drawn from a companion paper (not itself supplied), not absolute RU/DU/CU Watts, so it gives no numeric constant usable here — cited only as further qualitative corroboration of the centralization trade-off's direction, joining Rony et al. 2021 already on record above.
...
**2026-08-30 literature check, part 9**: obtained two related analytical modeling papers by Kuaban, Czachorski, Atmaca, and Czekalski ("Power models for O-RAN O-DU planning and optimisation" and "Analytical Power Efficiency Models for Open Radio Access Network (O-RAN)"). Key results: (1) the O-DU paper's own fitted static platform power (`P_plat,stat=50 W`) numerically matches this model's own `p_du_static_w=50.0` exactly in quantity-label and value, though its fuller idle-DU floor (including a 20 W accelerator and at least one idle core at 15 W, ~85 W total) sits above this model's own 50 W idle floor alone — a genuine but partial match, disclosed with that nuance. (2) A third independent real PA-efficiency value (`eta_PA=0.35`) further corroborates `pa_efficiency=0.25` (already validated in part 8). (3) A cited PA-share-of-RU-internal-power figure ("at least 64%," from Hao et al. 2024) brackets this model's own computed PA-share (~44% at `c=0`, ~73% at `c=2`) — the closest same-quantity RU-internal-power-share match found yet, especially at high centralization. (4) A new fronthaul-power quantification (`epsilon_FH=8%` of O-DU idle power, representing always-on fronthaul-interface hardware cost) is disclosed as useful context, not transplantable given this model's different additive (not multiplicative) fronthaul decomposition. (5) Both papers model O-DU/O-CU power with explicit sub-linear (pooling-discounted / logarithmic-in-users) scaling that this model's own linear-per-active-RU `compute_du_power()`/`compute_cu_power()` do not capture — disclosed as a genuine structural gap and candidate future-work item, not changed here (a functional-form change is a design decision, not a citation-driven constant fix). (6) Both papers independently reconfirm RU-power dominance with sub-linear DU/CU scaling — a third and fourth corroborating source. (7) The O-DU paper's own dynamic-core-shutdown savings (30.24%-59.42%) join the Bologna thesis's 53-60% as further precedent that real literature demonstrates savings well above this repo's own ≥15% target. See `oran_env/power_model.py`'s docstring for the full writeup, and `oran_env/traffic_model.py`'s docstring for a cross-track-only note on the O-DU paper's own dual-Gaussian traffic model (matches the C-RAN track's shape, not this track's trapezoidal design — not used to change anything here).
