MPHIL THESIS CONCEPT NOTE  —  v3.0
...
Document version: 3.0  (supersedes v2.0)

Prepared: 05 August 2026  ·  Status: Draft for supervisor review — response to review of v2.0

Purpose of this note

v2.0 was reviewed by the supervisor and returned with four blockers (B1–B4), six strongly-recommended items (S1–S6) and six advisory items (A1–A6). This version resolves every blocker and strongly-recommended item, and addresses each advisory item at the level the reviewer requested ("even if full benchmarking is out of scope" for A6). Section 0 maps each review item to the section that resolves it, so the supervisor can verify compliance without re-reading the whole document. Everything in v2.0 that the review did not flag (the system model, the MDP formulation, the core network architecture) is carried forward unchanged; this note explains only what changed and why.
...
4.4  Synthesis: the research gap (revised per B1)

The C-RAN literature converges on the same structural limitation from three algorithmic directions — DDQN-plus-SOCP, ANN-plus-heuristic, DNN-plus-convex — and even the closest hybrid attempt (Zhou et al., 2023) uses two separately-optimized policies rather than one coupled network. The O-RAN-native DRL literature (Bordin et al., 2025; Sohaib et al., 2024) is more recent but still single-decision-type: PPO/DQN for discrete RF-frontend activation, or transfer learning across traffic classes, neither of which couples a discrete activation decision to a continuous power parameter in one network. The 2025 A3C-plus-Dueling-DQN C-RAN paper (Chuang et al., 2025) confirms that combining an actor-critic method with a discrete-action method is an active idea in the "5G C-RAN + DRL" space generally, but applies it to an unrelated decision problem (industrial data-scheduling, not RRH activation/power), so it neither anticipates nor weakens this thesis's specific contribution. Separately, the general DRL literature already has well-tested tools for exactly this discrete-plus-continuous-parameter structure (P-DQN, MP-DQN, branching, and — very recently — HyAR and HySoft), but they have not, to my knowledge, been applied to the C-RAN joint RRH-activation-and-power problem, nor benchmarked against the DDQN and ANN+GSBF baselines already established for it, nor framed against O-RAN's rApp/E2/O1 interface split. That is the gap this thesis targets: a branching, multi-pass, twin-critic parameterized-DQN framework for C-RAN energy efficiency, evaluated head-to-head against the exact baselines from Iqbal et al. (2021) and Fathy et al. (2021), against P-DQN/MP-DQN directly (S2), against the simpler continuous-relaxation (pure-DDPG) alternative from the v1 concept note, and positioned as a deployable O-RAN rApp (S1) rather than a purely simulation-bound artifact.
...
| 2 | Literature Review | Substantially drafted | Fold in §4.2–4.4 of this note (updated DRL-building-blocks table, O-RAN subsection, revised novelty synthesis); target reference count now ~23, within the reviewer's 20–30 range |
