...
| TS-MP-DQN | Twin-Smoothed MP-DQN — extends MP-DQN with clipped double Q-learning and target-policy smoothing to fix the two failure modes DDPG-style parameter learning introduces into MP-DQN. **Standalone primary source not independently located** (found referenced within the related-work discussion of Fan et al., arXiv:2110.12618); flagged for verification before the thesis bibliography is finalized. | Same TD3-style overestimation fix as adopted independently here via Fujimoto et al. (2018) directly on the twin critics — confirms this fix is recognized as necessary in the parameterized-action-space literature specifically, not just the general actor-critic literature |
| CP-DQN | Yan, Chen, Xu, Wang & Peng (2025), *ScienceDirect* (journal name to be confirmed) | Adds a parameter-fluctuation-restriction (PFR) constraint so a branch's continuous parameter doesn't oscillate between adjacent timesteps when the discrete choice is unstable; a candidate refinement for the switching-cost-heavy reward in Section 10.2, noted as future work rather than adopted now |
...
"OREO": Open RAN Energy Optimisation via Deep Reinforcement Learning for 6G Networks. (2025). **Author list not yet independently confirmed from the located source — verify before finalizing the thesis bibliography, per the same treatment given to HySoft (§4.2).**

Yan, C., Chen, S., Xu, J., Wang, X., & Peng, Z. (2025). Hybrid Reinforcement Learning in parameterized action space via fluctuates constraint (CP-DQN). *ScienceDirect*, article identifier S0952197625025308. **Journal name/volume to be confirmed.**

Twin-Smoothed Multi-Pass Deep Q-Network (TS-MP-DQN): extends MP-DQN (Bester et al., 2019) with clipped double Q-learning and target-policy smoothing. **Standalone primary source not independently located for this revision** (identified only via its mention in the related-work section of Fan et al., arXiv:2110.12618); verify before citing directly in the thesis.
