...
> **Verification note (Ethical AI Rule, `docs/rules.md` §10; updated per the same direct-source standard applied after the OREO retraction, §4.3):** HySoft's authorship (Jianlin Lu, Yanzhao Jia, Daniel Görges) and venue (IFAC-PapersOnLine, ICONS 2025) are now **corroborated by an independent evidence chain**, not just a repeated search snippet: the exact PII (S2405896325027430) consistently resolves to the same title, venue, and benchmark set across multiple independently-phrased searches; Daniel Görges has his own, separately-verifiable ScienceDirect author profile (author ID 23569037600) that surfaces specifically in connection with this PII; and Görges is an independently confirmable real academic (control systems/ML research group, Rheinland-Pfälzische Technische Universität Kaiserslautern-Landau, with his own DBLP and Google Scholar profiles), consistent with an ICONS/IFAC (controls-community) venue. This is a materially stronger basis than the retracted OREO citation, which never resolved to any stable identifier. That said, every direct fetch attempt against the primary source itself (the ScienceDirect article page, Görges' DBLP page, and his Google Scholar page) returned HTTP 403 in this environment, so this is corroboration from independent secondary signals, not a confirmed primary-source read. The candidate should do one final check via institutional/library access to the ScienceDirect page before the thesis bibliography is finalized — the same standard now applied to every reference in this note per the correction in §4.3.
...
Lu, J., Jia, Y., & Görges, D. (2025). HySoft: Maximum Entropy Deep Reinforcement Learning with Hybrid Action Spaces. *IFAC-PapersOnLine*, 7th IFAC Conference on Intelligent Control and Automation Sciences (ICONS 2025), Padova, Italy. (ScienceDirect PII S2405896325027430; exact volume/pages not yet confirmed.) **Authorship corroborated via an independent evidence chain (Görges' separately-verifiable ScienceDirect author profile and academic identity at RPTU Kaiserslautern-Landau) but not confirmed by a direct primary-source read — every fetch attempt against ScienceDirect, DBLP, and Google Scholar returned HTTP 403 in this environment. Do one final check via institutional/library access before citing definitively (see §4.2 verification note).**
