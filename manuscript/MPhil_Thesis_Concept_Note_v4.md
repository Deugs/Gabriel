...
| G2 | O-RAN DRL energy work: OREO, ES-xApp, federated TD3, EExApp | Added — **EExApp (INFOCOM 2026) and Qazzaz et al.'s OREO (2026, retraction reversed and primary-source-verified) are the two closest published related works found and are discussed explicitly, not just listed** | §4.3, §4.4 |
...
Mobile data traffic continues to grow, and the Radio Access Network (RAN) is consistently reported as the dominant contributor to overall network energy consumption — GSMA benchmarking data and independent industry surveys converge on roughly 70–80% of total network power draw, commonly cited around 73%. Lassoued & Boujnah's (2026) recent comprehensive survey of 5G energy-efficiency strategies confirms this remains a live, actively-researched problem rather than a settled one, surveying the field's progression from traditional resource allocation, energy harvesting and BS sleep modes through to machine-learning, deep-reinforcement-learning and self-organizing-network approaches — the same progression this thesis's own approach sits within. Cloud Radio Access Network (C-RAN) architectures address part of this by centralizing baseband processing in a shared BBU pool while distributing low-cost Remote Radio Heads (RRHs), but the continuous operation of densely deployed RRHs and their fronthaul links still represents a large, largely static energy cost.
...
| HySoft | Lu, J., Jia, Y., & Görges, D. (2025). 7th IFAC Conference on Intelligent Control and Automation Sciences (ICONS 2025), Padova, Italy, Sept. 15-18, 2025. *IFAC-PapersOnLine* (ScienceDirect PII S2405896325027430). **Corroborated, not directly primary-source-confirmed — see verification note below.** | A 2025 maximum-entropy (soft) extension of P-DQN/MP-DQN with Q-value rescaling for exploration-exploitation balance; reported to outperform P-DQN/MP-DQN on standard hybrid-action benchmarks (Platform, Moving, Robot Soccer Goal, Catch Point); considered and not adopted (§10.1) |
...
> **Verification note (Ethical AI Rule, `docs/rules.md` §10; updated per the same direct-source standard applied when the Qazzaz et al. OREO citation was first checked, §4.3):** HySoft's authorship (Jianlin Lu, Yanzhao Jia, Daniel Görges) and venue (IFAC-PapersOnLine, ICONS 2025) are now **corroborated by an independent evidence chain**, not just a repeated search snippet: the exact PII (S2405896325027430) consistently resolves to the same title, venue, and benchmark set across multiple independently-phrased searches; Daniel Görges has his own, separately-verifiable ScienceDirect author profile (author ID 23569037600) that surfaces specifically in connection with this PII; and Görges is an independently confirmable real academic (control systems/ML research group, Rheinland-Pfälzische Technische Universität Kaiserslautern-Landau, with his own DBLP and Google Scholar profiles), consistent with an ICONS/IFAC (controls-community) venue. This remains a weaker basis than a genuine primary-source read: the Qazzaz et al. OREO citation (§4.3) was initially retracted on the same "could not resolve via search" grounds this HySoft citation currently rests on, and that retraction later turned out to be wrong once the actual paper was obtained and read directly — a caution against treating "does not resolve via search" as proof of non-existence, and a reminder that corroboration-by-secondary-evidence (this HySoft note) is not the same standard as a primary-source read (the OREO correction, §4.3). That said, every direct fetch attempt against the primary source itself (the ScienceDirect article page — retried and still blocked — Görges' DBLP page, his Google Scholar page, IFAC-PapersOnLine's own listing, and RPTU Kaiserslautern's KLUEDO open-access repository) returned either HTTP 403 or no matching entry in this environment, so this is corroboration from independent secondary signals only, never a confirmed primary-source read. Reaching the ScienceDirect page itself requires a real institutional/library login, which this environment does not have. **Action required from the candidate, not resolvable by further automated search:** open `https://www.sciencedirect.com/science/article/pii/S2405896325027430` directly via KNUST's (or any other) library subscription and confirm the title, author list, and volume/pages before this citation is finalized in the thesis bibliography — the same standard now applied to every reference in this note per the correction in §4.3.
...
| Qazzaz, Salama, Hafeez & Zaidi (2026), *IEEE Open Journal of the Communications Society*, DOI 10.1109/OJCOMS.2026.3685554 — **"OREO," retraction reversed and primary-source-verified (v4.0)** | A PPO-based rApp-xApp framework jointly determining RU activation states and multi-criteria user-association weights, deployed hierarchically (an energy-efficiency rApp in the Non-RT RIC, coordinating with an xApp in the Near-RT RIC); trained with high-fidelity Sionna ray-tracing channel models and an ML surrogate for real-time inference, evaluated against five baselines (Always-On, Random, Load-Based, Greedy RSRP, Proportional Fair) and against SAC/TD3 as alternative RL algorithms; reports 34.6% energy reduction vs. Always-On (3028 W → 1980 W) at a cost of only 0.36 percentage points of outage (0.89% vs. 0.53%) | Genuinely couples RU activation and user-association through **one** PPO policy network — but its "discrete" RU-activation decision is a **continuous relaxation** (a sigmoid output thresholded at 0.5 post-hoc), not a true discrete decision from a dedicated discrete head, and its continuous decision is association weights, not transmit power. Discussed explicitly, not just tabulated, in §4.4 alongside EExApp |
| Mungari, Puligheddu, Garcia-Saavedra & Chiasserini (2024), *IEEE INFOCOM*, arXiv:2405.18198 — **a second, unrelated real paper that also happens to be named "OREO"** | An O-RAN xApp orchestrator: shares xApps across services with semantically-equivalent functions to maximize the number of concurrently-deployed RAN services while minimizing overall xApp-execution energy/resource overhead, via a multi-layer graph model | A different axis from this thesis (service/xApp-deployment orchestration efficiency, not a discrete RU-activation-plus-continuous-power control policy); kept as an O-RAN-operational-efficiency reference, not a close architectural comparator |
...
| Eskandarinia, Vardakas, Famitafreshi & Verikoukis (n.d.) — manuscript supplied directly by the candidate; **venue/DOI not visible in the supplied copy, publication status unconfirmed** | A single DQN agent, positioned within the Near-RT RIC, selecting a joint (radio-unit, discretized transmit-power-level) action per user in a cell-free massive-MIMO O-RAN deployment, trading energy efficiency against spectral efficiency via a tunable weighting coefficient α; evaluated against Max-SE, Min-Power-Greedy and Random baselines at 12/14/16 users | A flat DQN over a combined (RU, power-level) action space — power is discretized to three candidate levels here, not continuous, and the two choices are not architecturally decoupled via branching or parameterization. Confirms, in a CF-mMIMO O-RAN setting close to this thesis's own, that a flat-DQN approach to this joint decision faces the same scaling ceiling Section 10.3.1 already argues against |
...
> **Correction (v4.0, retraction reversed):** an earlier draft of this note retracted a citation to "OREO: Open RAN Energy Optimisation via Deep Reinforcement Learning for 6G Networks" (PPO rApp, Sionna ray-tracing, 34.6% energy reduction, 0.89% outage, attributed to Qazzaz, Salama, Hafeez & Zaidi), on the grounds that it did not resolve to any real document on a direct IEEE Xplore/arXiv check at the time. The candidate has since obtained and read the paper directly — IEEE Open Journal of the Communications Society, Vol. 67 (2026), pp. 4165–4182, DOI 10.1109/OJCOMS.2026.3685554, received 14 March 2026, published 20 April 2026 — and every detail matches exactly: the PPO rApp-xApp architecture, the Sionna ray-tracing channel model, and the 34.6%/0.89% headline figures. **The retraction was itself the mistake, not a correct application of the Ethical AI Rule.** The paper is genuinely very recent — accepted and published in early-to-mid 2026 — which is the most likely reason it did not resolve via search when the earlier draft's verification attempt was made, not evidence that it doesn't exist. This is now a primary-source-confirmed reference, a stronger verification standard than the HySoft citation's independent-secondary-evidence-only standard (§4.2), since the actual document (not a search summary or a corroborating profile) has been read directly. There genuinely are two distinct, unrelated papers that happen to share the "OREO" name — Qazzaz et al.'s energy-optimization rApp-xApp framework (restored above) and Mungari et al.'s xApp-orchestration system (also above, INFOCOM 2024) — both real, both cited, disambiguated explicitly wherever either is discussed. The general standard the original retraction note established still holds — a located primary source (DOI, arXiv ID, or IEEE document number) is what gates a reference in this note — it has simply now actually been located and read, rather than merely searched for and not found.
...
**Qazzaz et al.'s OREO (2026) is a second close comparator requiring the same direct reckoning as EExApp, not a passing mention** (its earlier retraction in this note, now reversed — see §4.3's correction note — was itself an error, not evidence the paper is a weak or distant comparator). OREO genuinely couples a discrete-style decision (RU activation) and a continuous decision (multi-criteria user-association weights) through **one** PPO policy network — architecturally closer to this thesis's single-coupled-network approach than EExApp's two-actor-critic-pair design. The decisive difference is exactly the one Section 10.3.1 already argues matters: OREO's "discrete" RU-activation decision is a **continuous relaxation** — a sigmoid output thresholded at 0.5 after the fact, not a genuine discrete decision from a dedicated discrete head — the same PA-DDPG-style approach (Hausknecht & Stone, 2016) this thesis keeps only as its own pure-DDPG baseline (RQ3), specifically to test whether a true discrete decision beats it, rather than adopting continuous relaxation as the primary design. OREO also couples RU activation to user-association weights, not to transmit power, so it does not address this thesis's exact joint decision (activation-plus-power). That a rigorous, 2026, ray-tracing-validated, hierarchically-deployed O-RAN system still resorts to continuous relaxation for its discrete decision is itself supporting evidence that a genuinely discrete, branching, parameterized-action-space approach to this specific joint-decision problem remains unaddressed in the published literature — precisely this thesis's gap.

Mungari et al.'s OREO (2024) — a distinct, unrelated paper that happens to share the same name — and ES-xApp (Wang et al., 2024) are less direct competitors, on a different axis again: Mungari et al.'s OREO addresses xApp-deployment/sharing efficiency at the service-orchestration layer, not RU-activation-plus-power control, and ES-xApp is discrete-only (radio-card sleep, no continuous power/bandwidth decision). Sohaib et al. (2024) and the federated-TD3 work (Liang et al., 2026) address yet another axis (traffic-class transfer learning; federated/multi-agent scaling) rather than the discrete-continuous coupling question itself.

Separately, the general parameterized-action-space DRL literature has well-tested tools for exactly this discrete-plus-continuous-parameter structure (P-DQN, MP-DQN, branching, and — very recently — HyAR, HySoft, TS-MP-DQN and CP-DQN), but none of them, to my knowledge, has been applied to the C-RAN or O-RAN joint RRH/RU-activation-and-power problem specifically, nor benchmarked against the DDQN and ANN+GSBF baselines already established for C-RAN, nor framed against O-RAN's rApp/E2/O1 interface split. **That is the gap this thesis targets**, restated precisely in light of EExApp and Qazzaz et al.'s OREO: a branching, multi-pass, twin-critic *single coupled network* for joint RRH activation and power control, evaluated head-to-head against the exact baselines from Iqbal et al. (2021) and Fathy et al. (2021), against P-DQN/MP-DQN directly (S2), against the simpler continuous-relaxation (pure-DDPG) alternative from the v1 concept note — the same continuous-relaxation choice OREO itself makes for its discrete decision — and positioned as a deployable O-RAN rApp (S1) — with EExApp's dual-actor-dual-critic design and OREO's single-network-but-continuous-relaxed design named in Chapter 2 as the two nearest comparable architectures this thesis's coupled-network, true-discrete-decision claim must be defended against, not merely listed alongside.
...
EE(t) = Σu∈U Cu(t)  ⁄  Ptotal(t)

**Correction, v4.0**: an earlier draft of this equation carried a spurious `B ×` (BBU-count) divisor in the denominator, which does not appear in `cran_env/cran_env.py`'s implementation (`ee_mbit_per_joule = total_throughput_mbps / (p_total + 1e-6)`) or in `docs/thesis_guide.md`'s independently-stated `EE(t) = C_total(t) / P_total(t)`, and would have changed the metric's units away from the Mbit/Joule figure Section 12.3 reports. Removed to match both.
...
r(t) = EE(t)  −  λ1 Σu∈U max(0, Du(t) − Cu(t))  −  λ2 Σr∈R |kr(t) − kr(t−1)|  −  λ3 P_FH(t)

|k_r(t)−k_r(t−1)| ∈ {0,1} is an exact switching-event count. λ1, λ2, λ3 are set per the methodology in Section 12.6 (S5). λ1=λ2=λ3=0 recovers a reward equivalent to Iqbal et al.'s EE(t), the standing sanity check.

**Correction, v4.0**: fronthaul power P_FH(t) already reduces EE(t) implicitly (it is one of the terms summed inside P_total(t)), but — as with switching, which is penalized both implicitly (switching power inside P_total(t)) and explicitly (the λ2 term above, on the switch *count*) — an earlier draft of the ablation study (Section 4.5 of `docs/thesis_guide.md`) called for a standalone, independently-ablatable fronthaul term that the implementation had not actually added. λ3 (`reward.gamma_fronthaul` in `config/default.yaml`, applied to P_FH(t) in kW) closes that gap: setting λ3=0 removes the explicit fronthaul penalty while leaving fronthaul power's implicit presence in EE(t) untouched, mirroring exactly how λ2=0 does the same for switching.
...
- The policy itself would be deployed as an **rApp** in the Non-RT RIC, issuing the O1 (discrete) decisions directly and the E2 (continuous) decisions as policy guidance consumed by a co-located xApp — the same rApp/xApp split used by Bordin et al. (2025) and by Qazzaz et al.'s OREO (2026, §4.3/§4.4), and specified generally by the O-RAN Alliance's own commercialization roadmap (O-RAN Alliance, 2021).
...
- λ3 (`gamma_fronthaul`, default 1.0) is chosen to keep the fronthaul penalty (`gamma_fronthaul * P_FH(t)/1000`, P_FH typically ~0.05-0.15 kW at this thesis's scenario sizes) on the same order of magnitude as EE(t) itself, so the term is numerically meaningful rather than negligible, while still remaining a comparatively small, non-dominant weight next to the QoS/switching terms. (**Correction, v4.0**: an earlier draft used 0.1, which made the term's contribution numerically negligible regardless of P_FH — too small to actually compete for the policy's attention even as a minor term, which defeats the point of the ablation in Section 4.5 of `docs/thesis_guide.md`.)
- All three are then refined via a coarse grid sweep (e.g., λ2 ∈ {0.01, 0.05, 0.1, 0.5, 1.0} at fixed λ1/λ3) reporting how EE, QoS-violation rate and switching frequency shift — the sensitivity analysis the reviewer requested — and the final operating point is the one that meets the QoS target (Section 12.7) at the lowest switching frequency.
...
2. Before committing to the full 10-seed × 11-method experiment matrix, a **short proxy sweep** is run at the smallest scenario (R=5, U=2, 100 episodes, 2 seeds) over the single most sensitive parameter per network — the branch/continuous-net learning-rate pair (currently 1×10⁻⁴ / 3×10⁻⁴, matching Section 12.2/`config/default.yaml`) and τ (currently 0.005) — varying each roughly half an order of magnitude up and down. This is a targeted sensitivity check, not a search for a new optimum: if the default operating point is not visibly unstable (critic loss diverging, reward collapsing) relative to the swept alternatives, the default is kept.
...
| Reproduced baselines don't match published numbers | **Partially validated, scope corrected (v4.0):** exact numeric agreement with Iqbal et al. (2021)'s and Fathy et al. (2021)'s published Tables/Figures cannot be checked in this environment — no primary-source access to either paper is available here (the same constraint documented for the HySoft citation, §4.2), and fabricating a specific "reported number" that can't be checked against a primary source would violate the project's own Ethical AI Rule (`docs/rules.md` §10). What is tested instead (`tests/test_baseline_paper_scenarios.py`): the DDQN+SOCP baseline reproduces Iqbal et al.'s headline *qualitative* claim (meaningfully less power than All-ON) at the paper's own R=5,U=2 and R=12,U=4 scenarios. An equivalent generalization test was attempted for ANN+GSBF and initially failed — on held-out scenarios, the trained ANN did not beat a naive constant predictor (MAE) nor show significant correlation with the exhaustive-search ground truth (Spearman/Pearson ≈0, p>0.4). **Root-caused and fixed**: the original features (`baselines/ann_gsbf.py`) mixed raw demand statistics (~O(10-100)) with unnormalized channel-gain magnitudes (~O(1e-5)) with no standardization, starving the smaller-scale features of gradient signal, and lacked any feature that directly related demand to achievable capacity. Added a reference-scenario capacity-vs-demand ratio feature (computable without a live environment object, so training and inference stay consistent) and standardized all features before training; held-out predictions now show a real, statistically significant, reproducible positive correlation with the true label (Pearson r ≈ 0.1-0.16, p<0.05 in most trials) — a genuine, if still modest, generalization result rather than the previous indistinguishable-from-noise one. See `tests/test_ann_gsbf_training.py::test_ann_gsbf_generalizes_on_held_out_scenarios` |
...
| 10-seed × 11-method evaluation matrix (up from 5×7) increases compute load | Absorbed into the extended timeline (Section 15); for the proposed method, CSI-robustness and generalization runs can reuse the already-trained per-seed checkpoint (`BranchingMPDQN.load_checkpoint()`, wired into `training/run_extended_sweeps.py` via its opt-in `run_csi_and_generalization` flag) instead of training a fresh evaluation-only model — genuinely no extra training for that method when enabled. This is not yet implemented for the DDQN/DDPG baselines, which still train a small (~30-episode) standalone model for these two evaluations, since `training/train_baselines.py` does not currently save baseline model weights to reuse |
...
| 2 | Literature Review | Substantially drafted | Fold in §4.2–4.4 of this note (updated DRL-building-blocks table, expanded O-RAN subsection including EExApp and Qazzaz et al.'s OREO, revised novelty synthesis); reference count now 34 (an exact count of §17; three references added post-review: Eskandarinia et al., Lassoued & Boujnah, and the O-RAN Alliance whitepaper), now above the reviewer's 20–30 range — flagged here rather than silently exceeded |
...
Lu, J., Jia, Y., & Görges, D. (2025). HySoft: Maximum Entropy Deep Reinforcement Learning with Hybrid Action Spaces. *IFAC-PapersOnLine*, 7th IFAC Conference on Intelligent Control and Automation Sciences (ICONS 2025), Padova, Italy. (ScienceDirect PII S2405896325027430; exact volume/pages not yet confirmed.) **Authorship corroborated via an independent evidence chain (Görges' separately-verifiable ScienceDirect author profile and academic identity at RPTU Kaiserslautern-Landau) but not confirmed by a direct primary-source read — repeated fetch attempts against ScienceDirect, IFAC-PapersOnLine, RPTU's KLUEDO repository, DBLP, and Google Scholar all returned HTTP 403 or no matching entry in this environment. This requires a real institutional/library login to resolve, which is an action item for the candidate, not something further automated search can complete (see §4.2 verification note).**
...
Qazzaz, M. M. H., Salama, A., Hafeez, M., & Zaidi, S. A. R. (2026). OREO: Open RAN Energy Optimization via Deep Reinforcement Learning for 6G Networks. *IEEE Open Journal of the Communications Society*, 67, 4165–4182. https://doi.org/10.1109/OJCOMS.2026.3685554 (University of Leeds). **Retraction reversed (v4.0):** this citation was previously retracted as unresolvable via IEEE Xplore/arXiv search; the candidate has since obtained and read the paper directly, confirming it is genuine and every detail (PPO rApp-xApp architecture, Sionna ray-tracing, 34.6% energy reduction, 0.89% outage) matches exactly — see the correction note in §4.3.

Mungari, F., Puligheddu, C., Garcia-Saavedra, A., & Chiasserini, C. F. (2024). OREO: O-RAN intElligence Orchestration of xApp-based network services. 2024 IEEE International Conference on Computer Communications (INFOCOM). arXiv:2405.18198. **Verified directly** — a second, unrelated paper that happens to also be named "OREO" (Politecnico di Torino, NEC Laboratories Europe, CNIT); see §4.3 for disambiguation from Qazzaz et al.'s OREO above.
...
**New in v4.0 (additional literature supplied by the candidate):**

Eskandarinia, M., Vardakas, J. S., Famitafreshi, G., & Verikoukis, C. (n.d.). DQRL-Based Dynamic Resource Allocation for Energy-Spectral Efficient Next-Generation Clustered Radio Access Networks. Manuscript supplied directly by the candidate. **Venue, publication date and DOI are not visible in the supplied copy — publication status is unconfirmed; the candidate should confirm these details (or that the paper remains unpublished) before this citation is finalized in the thesis bibliography**, per the same primary-source standard applied throughout this note (§4.2, §4.3).

Lassoued, N., & Boujnah, N. (2026). A Comprehensive Review of Energy Efficiency in 5G Networks: Past Strategies, Present Advances, and Future Research Directions. *Computers*, 15(1), 50. https://doi.org/10.3390/computers15010050

O-RAN Alliance. (2021). *O-RAN Minimum Viable Plan and Acceleration towards Commercialization* (White Paper, 29 June 2021). Primary-source specification for the rApp/xApp, Non-RT RIC/Near-RT RIC, and O1/E2 architecture this thesis's O-RAN positioning (Section 11) is framed against.
