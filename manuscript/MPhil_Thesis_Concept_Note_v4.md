...
| **Retraction, see note below** — "OREO" | — | The description previously here ("Open RAN Energy Optimisation via DRL for 6G Networks", PPO rApp, Sionna ray-tracing, 34.6%/0.89%, attributed to Qazzaz/Salama/Hafeez/Zaidi) could **not** be confirmed against any primary source on a direct IEEE Xplore check and is now believed to be a search-summarization artifact rather than a real, distinct paper — withdrawn; see the verification note immediately below the table |
| Mungari, Puligheddu, Garcia-Saavedra & Chiasserini (2024), *IEEE INFOCOM*, arXiv:2405.18198 — **the actual "OREO"** | An O-RAN xApp orchestrator: shares xApps across services with semantically-equivalent functions to maximize the number of concurrently-deployed RAN services while minimizing overall xApp-execution energy/resource overhead, via a multi-layer graph model | A different axis from this thesis (service/xApp-deployment orchestration efficiency, not a discrete RU-activation-plus-continuous-power control policy); kept as an O-RAN-operational-efficiency reference, not a close architectural comparator |
...
> **Correction (v4.0, post-publication verification):** an earlier draft of this table cited "OREO: Open RAN Energy Optimisation via Deep Reinforcement Learning for 6G Networks" (PPO rApp, Sionna ray-tracing, 34.6% energy reduction, 0.89% outage, attributed to Qazzaz, Salama, Hafeez & Zaidi). Asked to verify this directly on IEEE Xplore, that description did not resolve to any real document there or on arXiv; the search tool's summaries had repeated the same specific description across multiple independent queries without it ever attaching to a concrete DOI, arXiv ID, or IEEE document number. The *only* paper actually named "OREO" that is confirmed on IEEE Xplore and arXiv is Mungari, Puligheddu, Garcia-Saavedra & Chiasserini's xApp-orchestration paper (INFOCOM 2024, arXiv:2405.18198), now cited above in its place, correctly described. Repetition across search results is not independent confirmation — this citation is retracted rather than merely re-flagged, and the same standard (a located primary source: DOI, arXiv ID, or IEEE document number) is what should gate any *other* reference in this note before the thesis bibliography is finalized, not just the three the supervisor specifically asked about.
...
The verified OREO (Mungari et al., 2024) and ES-xApp (Wang et al., 2024) are less direct competitors, on a different axis again: OREO addresses xApp-deployment/sharing efficiency at the service-orchestration layer, not RU-activation-plus-power control, and ES-xApp is discrete-only (radio-card sleep, no continuous power/bandwidth decision). Sohaib et al. (2024) and the federated-TD3 work (Liang et al., 2026) address yet another axis (traffic-class transfer learning; federated/multi-agent scaling) rather than the discrete-continuous coupling question itself.
...
**Retracted (v4.0, post-publication verification):** "Qazzaz, M. M. H., Salama, A., Hafeez, M., & Zaidi, S. A. R. — OREO: Open RAN Energy Optimisation via Deep Reinforcement Learning for 6G Networks" did not resolve to any real document on a direct IEEE Xplore/arXiv check and is withdrawn; see the correction note in §4.3.

Mungari, F., Puligheddu, C., Garcia-Saavedra, A., & Chiasserini, C. F. (2024). OREO: O-RAN intElligence Orchestration of xApp-based network services. 2024 IEEE International Conference on Computer Communications (INFOCOM). arXiv:2405.18198. **Verified directly** — this is the actual, confirmed "OREO" (Politecnico di Torino, NEC Laboratories Europe, CNIT).
