...
class UpperReplayBuffer:
    ...
    def push(
        ...
        # cont_params: the (power_ratio, prb_share) actually in effect when
        # this transition's discrete decision was made and its reward
        # observed -- see update_upper()'s own comment on why this must be
        # replayed, not recomputed fresh, for the *current*-state Q value.
        self.buffer.append(
            (state, ru_on, split, cont_params, aggregated_reward, next_state, done)
        )

    def sample(self, batch_size: int):
        batch = random.sample(self.buffer, batch_size)
        states, ru_on, split, cont_params, rewards, next_states, dones = zip(*batch)
        return (
            torch.FloatTensor(np.array(states)),
...
class BMPPDQNAgent:
    ...
    def __init__(
        ...
        self._pending_cont_params = np.zeros((n_ru, 2), dtype=np.float32)
    ...
    def remember(
        ...
        if self.last_action_was_decision:
            ...
            # The continuous params actually in effect at this decision
            # step -- replayed as-is into the upper buffer (see
            # update_upper()'s own comment for why recomputing a fresh
            # value from the current param_net instead would be wrong).
            self._pending_cont_params = cont_params.copy()
        else:
            self._pending_upper_reward_sum += reward
    ...
    def update_upper(self, batch_size: Optional[int] = None) -> Dict[str, float]:
        ...
        *encoder*'s features through the ONLINE critic (self.critic), not
        ...
        states, ru_on, split, cont_params, rewards, next_states, dones = (
            self.upper_memory.sample(min(batch_size, len(self.upper_memory)))
        )
        states = states.to(self.device)
        ...
        # prediction: must be the historical x actually in effect when
        # `rewards` was observed for this transition -- replayed from the
        # upper buffer, not recomputed fresh from the current param_net.
        #
        # Fixed bug: this previously recomputed (power_ratio, prb_share)
        # fresh from the current param_net at `states`, on the reasoning
        # that doing so "keeps this self-consistent without needing to
        # store/replay historical continuous params." That reasoning was
        # backwards: `rewards` was generated by the environment using the
        # continuous params in effect *at the time this transition was
        # recorded*, not whatever the (constantly shifting, every-step-
        # updated) param_net predicts *now*. Fusing a fresh, unrelated x
        # into the critic's forward pass while regressing it against a
        # reward that has nothing to do with that x is a genuine label/
        # input mismatch, not a design simplification -- the critic can
        # only ever learn Q(s, ru_on, split) decoupled from x, since x
        # carries no consistent relationship to the target it's trained
        # against. update_lower()'s actor then performs gradient ascent on
        # this same critic's (essentially x-independent, noise-sensitive)
        # output, which is a plausible root cause of the continuous power
        # head's observed collapse to a degenerate near-zero output across
        # every seed tested (see docs/daily_log.md and the O-RAN power-
        # sensitivity investigation). Fixed by replaying the actual
        # (power_ratio, prb_share) stored in the upper buffer instead --
        # see tests/test_oran_agents.py for the validation this fix
        # actually changes training behavior, not just the code path.
        activation_q, split_q = self._multi_pass_q(upper_feat, cont_params)
