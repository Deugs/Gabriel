...
- Write `tests/test_baselines.py` validating each baseline against its expected behavior (e.g. convex baseline matches a hand-computed reference solution; DDQN outperforms All-ON at Iqbal's exact studied scenarios — a specific reported power-savings % is not used as a pass/fail gate, since no primary-source access is available in this environment to verify it, per the Ethical AI Rule in `docs/rules.md` §10).
