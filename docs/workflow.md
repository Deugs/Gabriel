...
- [x] **Hyperparameter-tuning proxy sweep** (R=5, U=2, 100 episodes, 2 seeds; lr-pair and τ each varied ~half an order of magnitude, per Concept Note v4.0 §12.11/G9) — first run at full scale on 2026-08-05 (`data/results/proxy_sweep/`, since gone), but that run tested a since-superseded lr-pair (1e-3/1e-4) that no longer matched `config/default.yaml`'s real values (1e-4/3e-4) — a drift discovered 2026-08-13, with the code-level half (hardcoded fallback defaults) fixed 2026-08-15, but the sweep itself not re-run until **2026-08-29**. That 2026-08-29 run (`data/results/proxy_sweep_2026-08-29/`, session-local) genuinely covers today's real defaults: no variant crashed, and neither lr_discrete/lr_actor nor τ showed tail-critic-loss divergence relative to the swept alternatives, so both were **kept unchanged** per item 2 (this time evaluated by the round-15-fixed decision logic that actually compares against the alternatives, not just checks for a crash). Decision logged in `docs/daily_log.md` (2026-08-29 entry) per item 3. This gate is now genuinely satisfied for the config values actually in use — the full 10-seed × 11-method experiment matrix below remains unstarted, consistent with the Gantt.
